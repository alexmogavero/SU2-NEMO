...
void CIntegration::Space_Integration(CGeometry *geometry,
    ...
    switch (KindBC) {
      case EULER_WALL:
        if(config->GetKind_Centered_Flow() == GKS_BGK){
          solver_container[MainSolver]->BC_Euler_Wall(geometry, solver_container, numerics[CONV_TERM], config, iMarker);
        }else{
      ...
      case SUPERSONIC_INLET:
        if(config->GetKind_Centered_Flow() == GKS_BGK){
          solver_container[MainSolver]->BC_Supersonic_Inlet(geometry, solver_container, numerics[CONV_TERM], NULL, config, iMarker);
        }else{
      ...
      case SYMMETRY_PLANE:
        if(config->GetKind_Centered_Flow() == GKS_BGK){
          solver_container[MainSolver]->BC_Sym_Plane(geometry, solver_container, numerics[CONV_TERM], NULL, config, iMarker);
        }else{
  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++)
    switch (config->GetMarker_All_KindBC(iMarker)) {
      case ISOTHERMAL:
        if(config->GetKind_Centered_Flow() == GKS_BGK){
          solver_container[MainSolver]->BC_Isothermal_Wall(geometry, solver_container, numerics[CONV_TERM], NULL, config, iMarker);
        }else{
      ...
      case HEAT_FLUX:
        if(config->GetKind_Centered_Flow() == GKS_BGK){
          solver_container[MainSolver]->BC_HeatFlux_Wall(geometry, solver_container, numerics[CONV_TERM], NULL, config, iMarker);
        }else{
